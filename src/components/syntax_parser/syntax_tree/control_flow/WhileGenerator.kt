...
class WhileGenerator(conditionalGeneratorWord: Word, private val condition: ValueSyntaxTreeNode, private val isPostCondition: Boolean):
	...
	private val isNegatedCondition = conditionalGeneratorWord.type == WordAtom.UNTIL

	override fun toSemanticModel(scope: MutableScope): SemanticWhileGeneratorModel {
		return SemanticWhileGeneratorModel(this, scope, condition.toSemanticModel(scope), isPostCondition, isNegatedCondition)
	}

	override fun toString(): String {
		return "WhileGenerator [${if(isPostCondition) "post" else "pre"}${if(isNegatedCondition) " negated" else ""}] {${"\n$condition".indent()}\n}"
	}
}
