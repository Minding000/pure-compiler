...
object ByteNatives {
	...
	private fun unwrap(constructor: LlvmConstructor, wrappedLlvmValue: LlvmValue): LlvmValue {
		val thisPropertyPointer = constructor.buildGetPropertyPointer(
			context.byteTypeDeclaration?.llvmType, wrappedLlvmValue,
			context.byteValueIndex, "_propertyPointer")
		return constructor.buildLoad(constructor.byteType, thisPropertyPointer, "_primitiveValue")
	}

	private fun wrap(constructor: LlvmConstructor, primitiveLlvmValue: LlvmValue): LlvmValue {
		val newByteAddress = constructor.buildHeapAllocation(context.byteTypeDeclaration?.llvmType, "newByteAddress")
		val byteClassDefinitionPointer = constructor.buildGetPropertyPointer(
			context.byteTypeDeclaration?.llvmType,
			newByteAddress, Context.CLASS_DEFINITION_PROPERTY_INDEX, "byteClassDefinitionPointer")
		val byteClassDefinitionAddress = context.byteTypeDeclaration?.llvmClassDefinitionAddress
			?: throw CompilerError("Missing byte type declaration.")
		constructor.buildStore(byteClassDefinitionAddress, byteClassDefinitionPointer)
		val valuePointer = constructor.buildGetPropertyPointer(
			context.byteTypeDeclaration?.llvmType, newByteAddress,
			context.byteValueIndex, "valuePointer")
		constructor.buildStore(primitiveLlvmValue, valuePointer)
		return newByteAddress
	}
	...
	private fun increment(constructor: LlvmConstructor, llvmFunctionValue: LlvmValue) {
		...
		val thisPropertyPointer = constructor.buildGetPropertyPointer(
			context.integerTypeDeclaration?.llvmType, thisObjectPointer,
	...
	private fun add(constructor: LlvmConstructor, llvmFunctionValue: LlvmValue) {
		...
		val thisObjectPointer = context.getThisParameter(constructor)
		val thisPropertyPointer = constructor.buildGetPropertyPointer(
			context.byteTypeDeclaration?.llvmType, thisObjectPointer,
			context.byteValueIndex, "_thisPropertyPointer")
		val thisPrimitiveValue = constructor.buildLoad(constructor.byteType, thisPropertyPointer, "_thisPrimitiveValue")
		val parameterPrimitiveValue = unwrap(constructor, constructor.getParameter(llvmFunctionValue, Context.VALUE_PARAMETER_OFFSET))
		val result = constructor.buildIntegerAddition(thisPrimitiveValue, parameterPrimitiveValue, "_additionResult")
		constructor.buildStore(result, thisPropertyPointer)
		constructor.buildReturn()
